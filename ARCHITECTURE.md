...
- Stats: JSON snapshot of ingest rate, log lag and totals, shaped for amber.systems datapoints to poll every ~30s
...
The CT ecosystem has data sources, but nothing self-hosted and queryable with low operational friction:
...
`ctwatch` aims to be the simple, opinionated, MIT-licensed default for self-hosted CT mirroring. One Docker image plus a Postgres gives you a crt.sh-style read API. Adding it to a stack is one binding.
...
- **Not rate limiting.** Reverse proxy concern.

> `/v1/stream` (§8.6) fans out new precerts as they're written. It doesn't replicate Calidog's wire format and it doesn't replay history. Use it for live consumers; use the webhook path when you need guaranteed delivery.
...
All within a single Rust binary. State is in Postgres. There are no background queues outside the process; the webhook retry queue and backfill job state live in Postgres tables for durability.
...
The stack is Rust and Postgres. There is no Kafka, Redis, or external queue service.
...
This self-updates as logs come online. We don't hardcode URLs: when 2027 logs go live, ctwatch picks them up automatically; when 2025 logs decommission, it stops polling them. The only operator decision is which set to follow (default: all `usable` logs across all operators).
...
The CA/Browser Forum Baseline Requirements (§3.2.2.4, §4.2) require domain validation before the precert is submitted to CT. A precert is therefore a signed receipt from the CA confirming "this name was validated as belonging to whoever submitted the CSR." The final cert carries the same SANs minutes (or milliseconds) later. Storing both doubles disk for ~zero additional signal.

Edge case: some CAs deliver SCTs via OCSP or TLS extension and never submit a precert. This is essentially extinct for browser-trusted issuance in 2026: Let's Encrypt, DigiCert, Sectigo, Google Trust Services, ZeroSSL, and GlobalSign all use embedded-SCT precert flows. Estimated coverage loss: <1%. Acceptable.

### 6.5 Backfill

The live ingest path (§6.2) starts from the current STH. To populate the 90-day hot tier without waiting 90 days, `ctwatch backfill` walks each log backward from the current STH to a target timestamp and writes entries forward through the same writer pipeline.
...
1. **Find the entry-index for `--since`.** Binary search over `[0, tree_size)`:
   ...
   - Parse `MerkleTreeLeaf.TimestampedEntry.timestamp` (millis since epoch). This is the SCT timestamp rather than `not_before`. We use it because it's exposed in the leaf without parsing the cert, which is roughly 5× faster.
...
The live `serve` ingest worker keeps running during backfill. Both write to the same `certs` table; `ON CONFLICT DO NOTHING` handles deduplication. The live worker advances at the head of the log while the backfill worker walks the historical window. The only contention is on the writer channel, so backfill batches are tagged and the writer prioritises live entries when the channel approaches capacity.

Duration estimate at 2026 rates, fresh install with `--since=90d ago`: ~540M entries / ~20 logs / ~5 QPS each / ~1000 entries per request ≈ 90 minutes wall time. Real numbers depend on each log's rate-limit policy; Google's logs sustain >10 QPS, CloudFlare Nimbus throttles harder. The CLI prints per-log progress and an ETA.
...
Forward-compat: cert lifetimes shrink to 200 days (Mar 2026), 100 days (2027), 47 days (2029). Each reduction increases the renewal rate (same domain space, more issuances), so hot-tier storage scales linearly with issuance rate. By 2029, expect roughly 5× the 2026 ingest rate (~30M unique precerts/day). Hot tier at 2029 rates: ~500 GB compressed, still single-disk feasible.
...
- The server upgrades to WebSocket and sends each newly-observed precert (post-write to `certs`) as a JSON text frame.
- When `?domains=` is set, the server filters on `registered_domains`: a precert is forwarded if any of its `registered_domains` is in the subscription set.
- Best-effort and lossy. If a subscriber can't keep up, the server drops it with close code `1013` (`try again later`) and reason `"subscriber lagged"` after a lag threshold (default 64 messages behind, tunable via `STREAM_MAX_LAG`). The writer never blocks.
- No durability. Drops on disconnect, no replay. Use the webhook path when you need guaranteed delivery.
- Heartbeat: the server sends a `ping` every 30s and clients should respond with `pong`. Idle disconnect after 90s without a `pong`.
...
```
GET  /v1/stats
200 → JSON snapshot of operational metrics, for clients that poll on a schedule
      and emit time-series points. Counters are monotonic since process start;
...
The shape is stable across versions: fields may be added but never removed or renamed. Consumers (amber.systems datapoints in particular) should poll every 30–60s and emit the gauges they care about.
...
A precert "matches" the watchlist if any element of `cert.registered_domains` exactly equals a watchlist entry. We match on `registered_domain` (eTLD+1), not arbitrary SANs, because:

- A precert for `foo.example.com` has `registered_domain = "example.com"` and matches a watchlist entry for `"example.com"`.
- This correctly catches wildcards: a precert for `*.example.com` has `registered_domain = "example.com"`.
...
Watchlist entries SHOULD be registered domains. If a user supplies `foo.example.com`, normalise to `example.com` and warn.
...
- After 7 failed attempts: mark `failed`, surface in `/v1/metrics` (`webhook_failures_total`). The event is not dropped; an operator can re-deliver via the admin endpoint.
...
At ~6M precerts/day and a typical small-org watchlist (10–1000 domains), the per-cert hash-lookup takes microseconds. For very large watchlists (>100k domains), a Bloom filter pre-check before exact lookup would help; the design supports adding this, but it's not needed in v1.

## 10. Configuration

All config is env-var-driven (12-factor). An optional YAML file (`-config /etc/ctwatch.yml`) overrides defaults; env vars override the file.
...
> **Canonical deployment is a self-contained Docker Compose stack.** The repo ships everything required to bring ctwatch up on a fresh host with a single `docker compose up -d` after cloning. The compose stack includes its own Postgres + TimescaleDB instance, so there is no external database dependency. Bare-metal installation is supported (§11.2) but is not the default path.

### 11.1 Canonical deployment: self-contained Docker Compose stack

The repo's `deploy/docker-compose.yml` is the supported deployment artefact. It defines:
...
The `ctwatch serve` subcommand runs migrations on startup automatically (idempotent), then begins ingest and starts the HTTP server. No separate migrate step is needed in the compose flow.
...
The exact sequence for bringing ctwatch up from nothing. Designed to be runnable by an agent on the target host without further context.
...
After 5–10 minutes, `/v1/healthz` should report `ok: true` with `total_certs > 0`. Without backfill, the full 90-day window fills in over 90 days of live ingest. Run `ctwatch backfill` to populate the historical window immediately.
...
The compose stack does not include a backup service. For production use, schedule a backup on the host:
...
The `musl`-linked binary has no glibc dependency, so it runs on any modern Linux without extra runtime install. The compose stack is the supported path; this exists for completeness.
...
JSON to stdout. Key fields: `ts`, `level`, `msg`, plus event-specific fields (`log_id`, `cert_hash`, `event_id`, etc.). Intended for journald + Loki/Grafana or similar.
...
- Security policy: `SECURITY.md` with disclosure email. No private-vuln embargo expected in v0.x given the threat model (read-only public CT data), but we document the process.
- Reproducible builds: pin `Cargo.lock`, `cargo build --locked` in CI. Optional: `cargo-deny` for licence + advisory enforcement on every PR.
...
1. **Self-contained quickstart works end-to-end:** from a fresh clone on a Linux host with Docker installed, the exact sequence in §11.1 (clone → set `DB_PASSWORD` → optional `chattr +C` on btrfs → `docker compose build` → `docker compose up -d`) brings the stack to healthy state within 5 minutes, with no additional manual steps and no external service dependencies.
...
14. Licence header on all source files.
...
`ctwatch` is deployed on `chrysalis` (the existing VM that hosts `iptoasn`) using the canonical self-contained Docker Compose stack documented in §11.1. No amber-specific deployment tooling is needed; amber consumes a vanilla ctwatch deployment over the cloudflared tunnel like any other HTTP service.
...
- `ctwatch` configured with `WEBHOOK_URL=https://api.asy.st/v1/internal/ctwatch/event`, `WEBHOOK_HMAC_SECRET=<shared>`; the Worker validates HMAC, looks up which user(s) watch the matched domain, and fans out user-facing notifications

Both internal endpoints are gated by a shared HMAC secret; the credential lives in wrangler secrets on the Worker side and in `ctwatch`'s env on the VM side, with no public exposure.
...
- `totals.certs_in_window`: size of the 90-day hot tier
- `totals.unique_names_400d`: size of the 400-day cold tier
- `stream.subscribers`: live consumers connected (useful for capacity planning)
- `webhooks.outbox_depth` and `outbox_failed`: how healthy the alert path is
- Per-log `lag_entries` and `lag_seconds` for the top-3 highest-volume logs

The poll source lives in the same Worker as A.3/A.4. The cron trigger and the storage shape are specified in the datapoints feature, not here.

### A.6 Watch feature is its own spec

The amber-side Watch product (alert delivery to users, UI, history view) is out of scope for this spec. We add only the internal endpoints `ctwatch` needs to connect to it. Speccing the Watch product is a follow-on.
